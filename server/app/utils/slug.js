...
module.exports = (str, $separator = '-') => {
  ...
  for (let $key in charsArray) {
    str = str.replace(new RegExp(`${[].concat(charsArray[$key]).join('|')}`, 'gi'), $key)
  }
  str = str.replace(/[^\x20-\x7E]/giu, '')
  // Convert all dashes/underscores into separator
  const $flip = $separator === '-' ? '_' : '-'
  str = str.replace(new RegExp('[' + pregQuote($flip) + ']+', 'g'), $separator)
  // Remove all characters that are not the separator, letters, numbers, or whitespace.
  str = str.replace(new RegExp('[^' + pregQuote($separator) + 'a-z0-9\\s]+', 'gi'), $separator)

  // Replace all separator characters and whitespace by a single separator
  str = str.replace(new RegExp('[' + pregQuote($separator) + '\\s]+', 'g'), $separator)
